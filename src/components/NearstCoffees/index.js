...
const NearstCoffees = (props) => {
  ...
  async function loadCoffeePlace(store) {
    const { google_place_id } = store
    const response = await EstablishmentService.show(google_place_id)

    props.setPlace(response.data.result)
  }
...
​
      <Body>
        ...
        <hr />
        {
          stores.map(store => {
            return (
              <EstablishmentItem key={store.name} onClick={() => loadCoffeePlace(store) }>
